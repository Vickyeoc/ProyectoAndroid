    ...
    if(i==max){
         if(gano.isVisible){ gano.visibility= View.GONE}
            ...
            lifecycleScope.launch {
                try {
                    val envio = ResultadosRequest(codigoSecreto = codigoSecreto, intento = intento)
                    val respuesta = api.enviar(envio)
                    correcto = respuesta.acertados
                    noCorrecto = respuesta.parciales
